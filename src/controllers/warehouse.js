...
exports.get_my_warehouse = async (req, res) => {
  try {
    ...
    const warehouse = await Warehouse.findById(req.warehouse._id)
      .populate("warehouse_employees")
      .populate("received_transactions_history")
      .populate("sent_transactions_history");
    return response.response_success(res, response.OK, warehouse);
  } catch (err) {
    err.file = "warehouse.js";
...
exports.send_transaction_to_warehouse = async (req, res) => {
  try {
    ...
    if (
      !warehouse.inwarehouse_transactions_to_warehouse.includes(transaction._id)
    )
      return response.response_fail(
        res,
        response.NOT_FOUND,
        "transaction not in warehouse list to ship to warehouse"
      );
    const destination_transaction_spot = await TransactionSpot.findById(
      transaction.destination_transaction_spot
    );
    if (!destination_transaction_spot)
      return response.response_fail(
        res,
        response.NOT_FOUND,
        "transaction doesnt have destination transaction spot"
      );
    if (!destination_transaction_spot.warehouse)
      return response.response_fail(
        res,
        response.NOT_FOUND,
        "destination transaction spot doesnt have warehouse"
      );
    await Warehouse.findByIdAndUpdate(warehouse._id, {
      $pull: { inwarehouse_transactions_to_warehouse: transaction._id },
      $addToSet: { sent_transactions_history: transaction._id },
    });
    await Warehouse.findByIdAndUpdate(destination_transaction_spot.warehouse, {
...
exports.send_transaction_to_transaction_spot = async (req, res) => {
  try {
    ...
    if (
      !warehouse.inwarehouse_transactions_to_transaction_spot.includes(
        transaction._id
      )
    )
      return response.response_fail(
        res,
        response.NOT_FOUND,
        "transaction not in warehouse list to ship to transaction spot"
      );
    await Warehouse.findByIdAndUpdate(warehouse._id, {
      $pull: { inwarehouse_transactions_to_transaction_spot: transaction._id },
      $addToSet: { sent_transactions_history: transaction._id },
    });
    await TransactionSpot.findByIdAndUpdate(
      transaction.destination_transaction_spot,
...
exports.receive_transaction_from_warehouse = async (req, res) => {
  try {
    ...
    if (
      !warehouse.unconfirm_transactions_from_warehouse.includes(transaction._id)
    )
      return response.response_fail(
        res,
        response.NOT_FOUND,
        "transaction not in waiting from warehouse list"
      );
    if (
      !warehouse.transaction_spots.includes(
        transaction.destination_transaction_spot
      )
    ) {
      await Warehouse.findByIdAndUpdate(warehouse._id, {
        ...
        $addToSet: {
          inwarehouse_transactions_to_warehouse: transaction._id,
          received_transactions_history: transaction._id,
        },
      });
      return response.response_fail(
        res,
...
exports.receive_transaction_from_transaction_spot = async (req, res) => {
  try {
    ...
    if (
      !warehouse.unconfirm_transactions_from_transaction_spot.includes(
        transaction._id
      )
    )
      return response.response_fail(
        res,
        response.NOT_FOUND,
        "transaction not in waiting from transaction spot list"
      );
    if (
      !warehouse.transaction_spots.includes(
        transaction.destination_transaction_spot
      ...
      await Warehouse.findByIdAndUpdate(warehouse._id, {
        $pull: {
          unconfirm_transactions_from_transaction_spot: transaction._id,
        },
        $addToSet: {
          inwarehouse_transactions_to_warehouse: transaction._id,
          received_transactions_history: transaction._id,
