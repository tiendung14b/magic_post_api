...
exports.create_transaction = async (req, res) => {
  try {
    ...
    const destinationTransactionSpot = await TransactionSpot.findById(destination_transaction_spot)
    if (!sourceTransactionSpot) {
      return response.response_fail(res, response.NOT_FOUND, 'Source transaction spot not found')
    }
    ...
    response.response_success(res, response.OK, 'Create transaction success')
